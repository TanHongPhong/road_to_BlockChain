...
//#region node_modules/react-router/dist/development/chunk-NISHYRIK.mjs
...
function matchRoutesImpl(routes, locationArg, basename, allowPartial) {
	let location$1 = typeof locationArg === "string" ? parsePath(locationArg) : locationArg;
	let pathname = stripBasename(location$1.pathname || "/", basename);
...
function createRouter(init) {
	...
	async function handleAction(request, location$1, submission, matches, scopedContext, isFogOfWar, initialHydration, opts = {}) {
		interruptActiveLoads();
		let navigation = getSubmittingNavigation(location$1, submission);
	...
	async function handleFetcherAction(key, routeId, path, requestMatches, scopedContext, isFogOfWar, flushSync, preventScrollReset, submission) {
		...
		let actionResult = (await callDataStrategy(fetchRequest, fetchMatches, scopedContext, key))[match.route.id];
	...
	async function handleFetcherLoader(key, routeId, path, matches, scopedContext, isFogOfWar, flushSync, preventScrollReset, submission) {
		...
		let result = (await callDataStrategy(fetchRequest, dsMatches, scopedContext, key))[match.route.id];
		if (fetchControllers.get(key) === abortController) fetchControllers.delete(key);
...
function createStaticHandler(routes, opts) {
	...
	async function queryRoute(request, { routeId, requestContext, dataStrategy, generateMiddlewareResponse } = {}) {
		...
		if (generateMiddlewareResponse) {
			...
			return await runServerMiddlewarePipeline({
				...
				return await generateMiddlewareResponse(async (innerRequest) => {
					let result2 = await queryImpl(innerRequest, location$1, matches, requestContext, dataStrategy || null, false, match, null, false);
		...
		let result = await queryImpl(request, location$1, matches, requestContext, dataStrategy || null, false, match, null, false);
	...
	async function submit(request, matches, actionMatch, requestContext, dataStrategy, skipLoaderErrorBubbling, isRouteRequest, filterMatchesToLoad, skipRevalidation) {
		...
		if (!actionMatch.route.action && !actionMatch.route.lazy) {
			...
			result = (await callDataStrategy(request, dsMatches, isRouteRequest, requestContext, dataStrategy))[actionMatch.route.id];
		...
		if (isErrorResult(result)) {
			let boundaryMatch = skipLoaderErrorBubbling ? actionMatch : findNearestBoundary(matches, actionMatch.route.id);
			return {
				...await loadRouteData(loaderRequest, matches, requestContext, dataStrategy, skipLoaderErrorBubbling, null, filterMatchesToLoad, [boundaryMatch.route.id, result]),
				statusCode: isRouteErrorResponse(result.error) ? result.error.status : result.statusCode != null ? result.statusCode : 500,
				actionData: null,
				actionHeaders: { ...result.headers ? { [actionMatch.route.id]: result.headers } : {} }
			};
...
var RenderErrorBoundary = class extends import_react.Component {
	...
	componentDidCatch(error, errorInfo) {
		if (this.props.unstable_onError) this.props.unstable_onError(error, errorInfo);
...
function shallowDiff(a, b) {
	...
	for (let key of aKeys) if (a[key] !== b[key]) return true;
	return false;
}
function UNSTABLE_TransitionEnabledRouterProvider({ router: router$1, flushSync: reactDomFlushSyncImpl, unstable_onError }) {
	let fetcherData = import_react.useRef(/* @__PURE__ */ new Map());
	let [revalidating, startRevalidation] = import_react.useTransition();
	let [state, setState] = import_react.useState(router$1.state);
	router$1.__setPendingRerender = (promise) => startRevalidation(async () => {
	...
	import_react.useLayoutEffect(() => {
		return router$1.subscribe((newState, { deletedFetchers, flushSync, viewTransitionOpts }) => {
			newState.fetchers.forEach((fetcher, key) => {
				if (fetcher.data !== void 0) fetcherData.current.set(key, fetcher.data);
			});
			deletedFetchers.forEach((key) => fetcherData.current.delete(key));
			if (!shallowDiff(state, newState)) return;
			if (flushSync) if (reactDomFlushSyncImpl) reactDomFlushSyncImpl(() => setState(newState));
	...
	return /* @__PURE__ */ import_react.createElement(import_react.Fragment, null, /* @__PURE__ */ import_react.createElement(DataRouterContext.Provider, { value: dataRouterContext }, /* @__PURE__ */ import_react.createElement(DataRouterStateContext.Provider, { value: {
		...state,
		revalidation: revalidating ? "loading" : state.revalidation
	} }, /* @__PURE__ */ import_react.createElement(FetchersContext.Provider, { value: fetcherData.current }, /* @__PURE__ */ import_react.createElement(Router, {
		basename,
		location: state.location,
...
function getPatchRoutesOnNavigationFunction(manifest, routeModules, ssr, routeDiscovery, isSpaMode, basename) {
	...
	return async ({ path, patch, signal, fetcherKey }) => {
		...
		await fetchAndApplyManifestPatches$1([path], fetcherKey ? window.location.href : path, manifest, routeModules, ssr, isSpaMode, basename, routeDiscovery.manifestPath, patch, signal);
	};
}
function useFogOFWarDiscovery(router$1, manifest, routeModules, ssr, routeDiscovery, isSpaMode) {
	import_react.useEffect(() => {
		...
		async function fetchPatches() {
			...
			try {
				await fetchAndApplyManifestPatches$1(lazyPaths, null, manifest, routeModules, ssr, isSpaMode, router$1.basename, routeDiscovery.manifestPath, router$1.patchRoutes);
			} catch (e) {
...
${matches.map((match, routeIndex) => {
			...
			invariant2$1(manifestEntry, `Route ${match.route.id} not found in manifest`);
...
//#endregion
//#region node_modules/react-router/dist/development/chunk-65XJMMLO.mjs
...
function createRoutesStub(routes, _context) {
	return function RoutesTestStub({ initialEntries, initialIndex, hydrationData, future }) {
		...
		if (routerRef.current == null) {
			...
			let patched = processRoutes(convertRoutesToDataRoutes(routes, (r) => r), _context !== void 0 ? _context : future?.v8_middleware ? new RouterContextProvider() : {}, frameworkContextRef.current.manifest, frameworkContextRef.current.routeModules);
		...
		let commonRoute = {
			...
			loader: route.module.loader ? async (args) => {
				...
				if (preRenderedData != null) {
					...
					let data2 = (await decodeViaTurboStream(stream, global)).value;
...
function derive(build, mode) {
	...
	let staticHandler = createStaticHandler(dataRoutes, { basename: build.basename });
...
var createRequestHandler = (build, mode) => {
	...
	return async function requestHandler(request, initialContext) {
		...
		if (!_build.ssr) {
			...
			else if (!_build.prerender.includes(decodedPath) && !_build.prerender.includes(decodedPath + "/")) if (url.pathname.endsWith(".data")) {
		...
		if (url.pathname === manifestUrl) try {
			return await handleManifestRequest(_build, routes, url);
		} catch (e) {
			handleError(e);
		...
		if (url.pathname.endsWith(".data")) {
			...
			if (isRedirectResponse(response)) response = generateSingleFetchRedirectResponse(response, request, _build, serverMode);
			...
			if (_build.unstable_getCriticalCss) criticalCss = await _build.unstable_getCriticalCss({ pathname });
...
function getFetchAndDecodeViaRSC(createFromReadableStream, fetchImplementation) {
	return async (args, basename, targetRoutes) => {
		...
		try {
			...
			for (let [routeId, data2] of Object.entries(payload[dataKey] || {})) results.routes[routeId] = { data: data2 };
...
function RSCHydratedRouter({ createFromReadableStream, fetch: fetchImplementation = fetch, payload, routeDiscovery = "eager", getContext }) {
	if (payload.type !== "render") throw new Error("Invalid payload type");
	let { router: router$1, routeModules } = import_react.useMemo(() => createRouterFromPayload({
		payload,
		fetchImplementation,
		getContext,
	...
	let [location2, setLocation] = import_react.useState(router$1.state.location);
	import_react.useLayoutEffect(() => router$1.subscribe((newState) => {
		if (newState.location !== location2) setLocation(newState.location);
	}), [router$1, location2]);
	import_react.useEffect(() => {
		if (routeDiscovery === "lazy" || window.navigator?.connection?.saveData === true) return;
		function registerElement(el) {
			let path = el.tagName === "FORM" ? el.getAttribute("action") : el.getAttribute("href");
			if (!path) return;
			let pathname = el.tagName === "A" ? el.pathname : new URL(path, window.location.origin).pathname;
	...
	return /* @__PURE__ */ import_react.createElement(RSCRouterContext.Provider, { value: true }, /* @__PURE__ */ import_react.createElement(RSCRouterGlobalErrorBoundary, { location: location2 }, /* @__PURE__ */ import_react.createElement(FrameworkContext.Provider, { value: frameworkContext }, /* @__PURE__ */ import_react.createElement(UNSTABLE_TransitionEnabledRouterProvider, {
		router: router$1,
...
function HydratedRouter(props) {
	...
	let [location$1, setLocation] = import_react.useState(router.state.location);
	import_react.useLayoutEffect(() => {
	...
	import_react.useLayoutEffect(() => {
		if (ssrInfo && ssrInfo.router) return ssrInfo.router.subscribe((newState) => {
			if (newState.location !== location$1) setLocation(newState.location);
		});
	}, [location$1]);
	...
	} }, /* @__PURE__ */ import_react.createElement(RemixErrorBoundary, { location: location$1 }, /* @__PURE__ */ import_react.createElement(RouterProvider2, {
		router,
